...
import {entries,records,contentHTML,dateLabel} from '@/lib/acervo';
import snapshot from '@/data/acervo.json';
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const e=entries.find(p=>p.slug===slug);return {title:e?e.title+' — FGC':'Publicação não encontrada — FGC',description:e?.excerpt.slice(0,160)}}
export default async function Article({params}:{params:Promise<{slug:string}>}){
 const {slug}=await params;const e=entries.find(p=>p.slug===slug);const record=records.find(p=>p.slug===slug);if(!e||!record)notFound();
 const docs=snapshot.media.filter(m=>m.post===e.id&&!m.mime_type.startsWith('image/'));
