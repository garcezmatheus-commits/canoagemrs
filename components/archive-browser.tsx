...
export default function ArchiveBrowser({entries,categories,initialCategory}:{entries:Entry[];categories:Category[];initialCategory:string}){
 ...
 const filtered=useMemo(()=>entries.filter(e=>{
  ...
  const categoryMatch=category==='todos'||(category==='governanca'&&(governanceSlugPages.includes(e.slug)||e.categoryIds.some(id=>governanceCategoryIds.includes(id))))||(category==='circulares'&&e.categoryIds.some(id=>circularesCategoryIds.includes(id)))||(cat&&e.categoryIds.includes(cat.id));
  return categoryMatch&&(year==='todos'||e.date.startsWith(year))&&normalize(e.title+' '+e.excerpt+' '+e.search).includes(normalize(query));
 }),[query,category,year,entries,categories,governanceCategoryIds,circularesCategoryIds]);
