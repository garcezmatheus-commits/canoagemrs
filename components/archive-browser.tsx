...
export default function ArchiveBrowser({entries,categories,initialCategory}:{entries:Entry[];categories:Category[];initialCategory:string}){
 ...
 const filtered=useMemo(()=>entries.filter(e=>{
  ...
  const governance=['diretoria','estatuto','estatuto-2024','estatuto-fgc','balanco','governanca','galeria-dos-ex-presidentes','institucional'];
  const categoryMatch=category==='todos'||(category==='governanca'&&(governance.includes(e.slug)||e.categoryIds.some(id=>[40,44,45,36,29].includes(id))))||(category==='circulares'&&e.categoryIds.some(id=>[21,36,29].includes(id)))||(cat&&e.categoryIds.includes(cat.id));
  return categoryMatch&&(year==='todos'||e.date.startsWith(year))&&normalize(e.title+' '+e.excerpt+' '+e.search).includes(normalize(query));
 }),[query,category,year,entries,categories]);
