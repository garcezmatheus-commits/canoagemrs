...
export default function SiteHeader(){
 ...
 return <header className="site-header"><Link ref={logo} href="/" className="logo-sticker" aria-label="FGC — início"><LogoMark/></Link><a href="#conteudo" className="skip-link">Pular para conteúdo</a><nav id="main-nav" className={`main-nav ${open?'is-open':''}`} aria-label="Principal">{[['A Federação','/#federacao'],['Competições','/#competicoes'],['Notícias','/#noticias'],['Acervo','/acervo']].map(([name,url])=><Link key={name} href={url} onClick={()=>setOpen(false)}>{name}</Link>)}</nav><a className="header-join" href="/#federado">Seja um federado <ArrowUpRight size={17}/></a><button className="menu-toggle" aria-label={open?'Fechar menu':'Abrir menu'} aria-controls="main-nav" aria-expanded={open} onClick={()=>setOpen(!open)}>{open?<X/>:<Menu/>}</button></header>;
}
